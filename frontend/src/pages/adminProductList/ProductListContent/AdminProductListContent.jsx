  ...
  const handleFilter = (e, filter, option) => {
    ...
    if (e.target.checked) {
      if (newFilter[filter.id]) {
        newFilter[filter.id].push(
          option.value || [option.minValue, option.maxValue]
        );
      } else {
        newFilter[filter.id] = [
          option.value || [option.minValue, option.maxValue],
        ...
        <div className="top-0 flex justify-between w-full px-4 py-4 mx-0 border-b-2">
          ...
          <div className="flex items-center justify-center gap-6">
            ...
            <Link
              ...
              className="flex gap-3 px-2 py-2 mx-auto text-xs text-center text-white duration-300 rounded btn-wide btn md:px-4 md:text-lg btn-neutral"
            >
              Add New Product
            </Link>
          </div>
...
export default AdminProductListContent;
